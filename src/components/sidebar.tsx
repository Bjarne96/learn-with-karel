...
const Topbar: React.FC<ISidebar> = (props) => {
    ...
    const returnButton = (name: string) => {
        const svgPaths = [{
            ...
            "className": "w-9 has-tooltip h-10 my-auto mr-3",
            ...
            "className": "w-8 has-tooltip h-10 my-auto mt-3",
            ...
            "className": "w-11 has-tooltip h-10 my-auto mr-l mt-2 mb-1",
        ...
        let container = ""
        if (btn.tabId == props.activeTab) btn.color = "fill-white text-white"
        if (btn.tabId == props.activeTab) container = container + "cursor-pointer flex flex-row items-center justify-center w-full h-12 border-l-4"
        else container = "cursor-pointer flex flex-row items-center w-full h-12 justify-center border-custom-blue border-l-4"
        return <div onClick={btn.onClick} className={container}>
            <div className={btn.className}>
                {typeof btn.text == "string" && <span className="tooltip rounded shadow-lg p-1 bg-code-grey text-white -mt-8">
                    {btn.text}
                </span>}
                <svg
            ...
            {btn.addtionalText != null && <p className={btn.color + " font-bold text-xl "}>&nbsp;{btn.addtionalText}</p>}
        </div>
    }
